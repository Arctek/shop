...
contract('Ownable', accounts => {
    ...
    beforeEach(() =>
        Ownable.new({ from: owner }).then(instance => contract = instance)
    );
    ...
    it('should allow owner to set owner', async () => {
        let txObject = await contract.setOwner(bob, { from: owner });
    ...
    it('should not allow non-owner to set owner', () =>
        web3.eth.expectedExceptionPromise(() => 
            contract.setOwner(bob, { from: bob, gas: gasToUse }), gasToUse)
    );

    it('should not allow owner to set owner to itself', () =>
        web3.eth.expectedExceptionPromise(() => 
            contract.setOwner(owner, { from: owner, gas: gasToUse }), gasToUse)
    );
});
